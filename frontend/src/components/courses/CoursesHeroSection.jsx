...
export default function CoursesHeroSection({ isSearching }) {
  return (
    <section className={`relative px-6 lg:px-8 overflow-hidden bg-surface-container-low flex flex-col md:flex-row items-center justify-between transition-all duration-500 ${
      isSearching ? 'py-10 lg:py-14' : 'py-16 lg:py-24 xl:py-32'
    }`}>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        <div className={`flex flex-col items-start transition-all duration-500 ${isSearching ? 'lg:col-span-12 space-y-4' : 'lg:col-span-7 space-y-6 lg:space-y-8'}`}>
          <h1 className={`font-headline font-bold tracking-tighter text-on-surface leading-tight transition-all duration-500 ${
            isSearching ? 'text-3xl lg:text-4xl' : 'text-4xl lg:text-5xl xl:text-7xl'
          }`}>
          ...
          {!isSearching && (
            <>
              <p className="font-body text-base lg:text-lg xl:text-xl text-on-surface-variant max-w-2xl leading-relaxed">
                Explore industry-relevant programs designed to make you job-ready with real skills, projects, and experience.
              </p>
              {/* Highlight Badges */}
              <div className="flex flex-wrap gap-3 lg:gap-4 font-label text-sm text-on-surface-variant">
                <span className="flex items-center gap-2 bg-surface-container px-3 lg:px-4 py-2 rounded-full border border-outline-variant/20 shadow-[0_0_20px_rgba(192,193,255,0.05)]">
                ...
                <span className="flex items-center gap-2 bg-primary-container/20 px-3 lg:px-4 py-2 rounded-full border border-primary/30 text-primary font-medium">
                  <span className="material-symbols-outlined text-base">payments</span> ₹2499 Only
                </span>
              </div>
              <Link to="/register">
                <button className="bg-gradient-to-r from-[#8A2BE2] to-[#a855f7] text-white font-headline font-bold text-base lg:text-lg px-8 lg:px-10 py-3 lg:py-4 rounded-full hover:shadow-[0_0_40px_rgba(138,43,226,0.3)] transition-all duration-300 transform hover:-translate-y-1">
                  Start Learning
                </button>
              </Link>
            </>
        ...
        {/* Hero Image / Visual Asymmetry — hidden when searching */}
        {!isSearching && (
