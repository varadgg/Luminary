...
export default function RegistrationFormSection() {
  ...
  return (
    <section className="w-full relative flex justify-end">
      <div className="w-full max-w-2xl p-4 md:p-5 rounded-[2rem] bg-gradient-to-b from-surface-container/80 to-surface-container-high/80 backdrop-blur-xl border border-white/10 shadow-[0_0_60px_rgba(0,0,0,0.6)] lg:max-h-[85vh] flex flex-col overflow-hidden">
        ...
        {status.type === 'success' && appId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
            <div className="bg-surface-container border border-primary/30 p-6 sm:p-8 rounded-3xl max-w-sm w-full shadow-2xl text-center relative overflow-hidden">
              ...
              <p className="text-sm text-on-surface-variant mb-3">Your Application ID:</p>

              {/* Application ID — Hero Element */}
              <div
                className="bg-black/40 border border-white/10 rounded-xl py-3 px-4 mb-3 cursor-pointer hover:border-primary/40 transition-colors group relative"
                onClick={handleCopyId}
                title="Click to copy"
              >
                <span className="text-xl font-mono font-bold tracking-wider text-primary">{appId}</span>
                <span className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs font-medium transition-all ${copied ? 'text-green-400' : 'text-white/30 group-hover:text-white/60'}`}>
                  {copied ? '✓ Copied!' : 'Copy'}
                </span>
              </div>

              <p className="text-xs text-white/50 mb-2">Please save this ID for future reference.</p>
              <p className="text-sm text-green-400/80 mb-4">✅ Your application has been submitted successfully.<br />✅ Our team will contact you shortly.</p>

              {/* Support Contact — Secondary Info */}
              <div className="bg-white/5 border border-white/10 rounded-xl p-3 mb-5">
                <p className="text-[11px] text-white/50 mb-1.5">Need help or have questions?</p>
                <a
                  href="mailto:luminaryacademyofficial@gmail.com"
                  className="text-xs text-purple-400 hover:text-purple-300 transition-colors font-medium break-all"
                >
                  📧 luminaryacademyofficial@gmail.com
                </a>
              </div>
        ...
        <form className="space-y-2.5 flex-grow overflow-y-auto" onSubmit={handleSubmit}>
          ...
          {/* Submit Button */}
          <button disabled={isLoading} className="w-full bg-gradient-to-r from-purple-500 via-indigo-500 to-cyan-500 text-white font-bold text-base md:text-lg py-2.5 rounded-2xl shadow-[0_0_40px_rgba(168,85,247,0.5)] hover:shadow-[0_0_60px_rgba(168,85,247,0.8)] transition-all duration-300 mt-3 uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed border border-white/20" type="submit">
            {isLoading ? (
